...
for (i in 1:length(unique(dsSPDF_amphibian@data$binomial))) {
  ...
  # What is the area of species i hull
  area_hull <- 
  ...
  intersect(map_i, hull_i)
  
  # Outputs
  # I have added the number and % of areas as some
  # species have > 1 type
  areas_amphibian[i, "Binomial"] <- species_i
  areas_amphibian[i, "AreaHull"] <- area_hull
  areas_amphibian[i, "AreaOverlaps"] <- no_areas
  areas_amphibian[i, "PercentOverlaps"] <- (no_areas/no_types) * 100
...
# Save output
write_csv(areas_amphibian, file = here("data/areas_amphibian"))
write_csv(areas_bird, file = here("data/areas_bird"))
write_csv(areas_mammal, file = here("data/areas_mammal"))
```
